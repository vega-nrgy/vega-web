...
const AUDIENCE = ['Hotels & Resorts', 'Fuel Stations', 'Shopping Centres', 'Highway Dhabas & Rest Stops', 'Commercial Complexes']

const STRIPE_STYLE = {
  backgroundImage: 'repeating-linear-gradient(-45deg, #e2eae9 0 16px, #e8efee 16px 32px)',
} as const

export function CommercialCharging() {
  return (
    <Section id="commercial" labelledBy="commercial-heading" className="mt-28 scroll-mt-24 bg-grey-soft">
      <div className="mx-auto max-w-7xl px-6 py-24 lg:px-8">
        ...
        <div className="mt-7 grid gap-16 lg:grid-cols-[1.15fr_1fr] lg:items-stretch">
          <div>
            <h2 id="commercial-heading" className="font-display text-[38px] font-semibold leading-[1.12] tracking-[-0.03em] text-ink">
              Power your business. Charge smarter.
            </h2>
            <p className="mt-4 text-[15px] leading-relaxed text-muted">
              For businesses looking to offer EV charging as a value-added service &mdash;
              whether you own a hotel, restaurant, mall, fuel station, or any commercial
              establishment along a highway corridor.
            </p>
            <ul className="mt-6 border-t border-[#dce4e3]">
              {CHECKLIST.map((item) => (
                <li key={item} className="flex gap-3 border-b border-[#dce4e3] py-3 text-[13.5px] font-medium leading-relaxed text-ink-soft">
            ...
            <ul className="mt-5 flex flex-wrap gap-2">
              {AUDIENCE.map((label) => (
                <li key={label} className="rounded-full bg-white px-3.5 py-2 text-[11.5px] font-semibold text-ink-soft">
                  {label}
                </li>
              ))}
            </ul>
          ...
          <div className="relative overflow-hidden rounded-media">
            <div style={STRIPE_STYLE} className="flex h-full min-h-[420px] items-center justify-center">
